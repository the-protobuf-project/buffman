...
func (c *FlatbuffersParser) Parse(ctx context.Context, opts options.ParseOptions) error {
	...
	report := c.processFiles(ctx, protoFiles, t)
...
func (c *FlatbuffersParser) processFiles(ctx context.Context, protoFiles []string, t *template.Template) *FileProcessingReport {
	...
	for _, protoFile := range protoFiles {
		...
		if err := utilities.InsertGeneratedComments(t.BuildHeader(protoFile), fbsPath); err != nil {
			report.recordFailure(protoFile, fmt.Sprintf("✗ Failed to add comment to %s: %v", protoFile, err))
			continue
		}
		report.recordSuccess(fmt.Sprintf("✓ Converted: %s", protoFile))
	}
	return report
}
